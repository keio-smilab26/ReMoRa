...
class LLaVATrainer(Trainer):
    # Default value for Tensor Parallelism flag (required by newer transformers versions)
    is_tp_enabled = False
    ...
    def create_accelerator_and_postprocess(self):
        ...
        # dispatch_batches and split_batches removed in accelerate 1.0+
        ...
        # Tensor Parallelism flag (required by newer transformers versions)
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if not isinstance(train_dataset, torch.utils.data.IterableDataset):
            ...
            # seed_worker signature: seed_worker(worker_id) in transformers 4.51+
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        if getattr(self.args, "tune_mm_mlp_adapter", False) or (
            ...
            if self.args.lora_enable:
                #First save adapter and compressor
                from transformers.trainer_utils import PREFIX_CHECKPOINT_DIR
                checkpoint_folder = f"{PREFIX_CHECKPOINT_DIR}-{self.state.global_step}"
                ...
                keys_to_match = ["mm_projector", "vision_resampler", "compressor", "mv_encoder", "mv_fuse_alpha", "image_newline"]
                if getattr(self.args, "use_im_start_end", False):
                    keys_to_match.extend(["embed_tokens", "embed_in"])
            ...
            #Then save the rest
            # Note: newer transformers removed 'metrics' parameter from _save_checkpoint
